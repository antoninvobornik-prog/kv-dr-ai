...
# Inicializace AI modelu (automatická detekce funkčního modelu)
...
# --- AI CHAT STRÁNKA ---
    ...
    st.caption("Ptejte se na organizaci Kvádr nebo na aktuální počasí.")
    
    st.sidebar.caption(f"Aktivní model: {st.session_state.model_name}")
        ...
        with st.chat_message("assistant"):
            with st.spinner("Kvádr AI přemýšlí..."):
                try:
                    ...
                    full_prompt = (
                        f"Jsi Kvádr AI, přátelský asistent. "
                        f"Zde jsou informace o nás: {kontext}. "
                        f"Aktuální počasí: {pocasi_txt}. "
                        f"Odpovídej česky. Uživatel píše: {prompt}"
                    )
                    ...
                    st.error(f"Omlouvám se, došlo k chybě: {e}")import streamlit as st
import pandas as pd
import google.generativeai as genai
import urllib.parse
import requests
